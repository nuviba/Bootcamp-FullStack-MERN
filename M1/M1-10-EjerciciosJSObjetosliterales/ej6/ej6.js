...
/* 6. La página tendrá el objeto ‘personas’ en el script. Al entrar alguien en la página se le
pregunta el nombre de la persona sobre la que se quiere datos. Si coincide el nombre con uno de los 7 en el objeto, mostrar los datos del siguiente modo:
a. Crea un div centrado tanto vertical como horizontalmente en el que
aparecerá la foto de la persona (picture), seguida de un h1 con el nombre y
párrafos para cada uno de los atributos (age, 
    email y address). El color de
fondo será el color de ojos (eyeColor)
En caso de que no haya persona con ese nombre, mostrar un mensaje al respecto. */

let nombre=window.prompt('Dime el nombre que buscas:');
let atributos;
let posicion;
let edad;
let correoe;
...
let color;
let prueba=false;
atributos=Object.keys(personas);

  for(let i=0;i<atributos.length;i++){
    if(nombre==personas[atributos[i]].name){
      posicion=i;
      prueba=true;
    }
   ...
   if(prueba==false){
      window.alert('No hay una persona con ese nombre:');
   }

   else{
   edad= personas[atributos[posicion]].age;
   correoe= personas[atributos[posicion]].email;
   dire=personas[atributos[posicion]].address;
   phot=personas[atributos[posicion]].picture;
   color=personas[atributos[posicion]].eyeColor;
   
   
   
   document.getElementById("nombre").innerHTML='NOMBRE: '+nombre;
   document.getElementById("años").innerHTML='EDAD: '+edad;
   document.getElementById("email").innerHTML='EMAIL :'+correoe;
   document.getElementById("direccion").innerHTML='DIRECCION:'+dire;
   document.getElementById("imag").setAttribute("src",phot);
   document.getElementById("ficha").style.background=color; }
  document.getElementById("ficha").style.textAlign="center";
  document.getElementById("ficha").style.width="40%";
